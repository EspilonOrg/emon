...
int config_parse_args(config_t *cfg, int argc, char *argv[])
{
    ...
    /* -i <port> with no positional port: register that port implicitly */
    if (cfg->interactive && cfg->input_port[0] && cfg->nports == 0) {
        int j = cfg->nports++;
        snprintf(cfg->ports[j], sizeof(cfg->ports[j]), "%s", cfg->input_port);
        const char *base = strrchr(cfg->ports[j], '/');
        snprintf(cfg->names[j], sizeof(cfg->names[j]), "%s",
                 base ? base + 1 : cfg->ports[j]);
    }
