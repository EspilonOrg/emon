...
/* Strip ANSI escape sequences for search comparisons */
static void strip_ansi(const char *src, char *dst, size_t dlen)
{
    size_t di = 0;
    for (const char *s = src; *s && di < dlen - 1; s++) {
        if (*s == '\033') {
            while (*s && *s != 'm') s++;
        } else {
            dst[di++] = *s;
...
static int line_matches(const char *line, const char *query)
{
    ...
    strip_ansi(line, plain, sizeof(plain));
...
static void render(int offset, int rows, int cols, const char *status_extra)
{
    pthread_mutex_lock(&s_mutex);
